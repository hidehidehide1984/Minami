...
**図（SVG）を付けたい場合**は、任意の `svg` フィールドにインラインSVGの文字列を入れる
（問題文の下に表示される。`app.js` の `renderQuestion` が対応済み）:
```js
{ id: "y2024b_fig2", category: "zukei", level: 2, type: "choice",
  q: "図の三角形で、2つの角が65°と40°です。残りの角（？）は何度？",
  svg: '<svg viewBox="0 0 260 140" xmlns="http://www.w3.org/2000/svg">…</svg>',
...
- SVGは `viewBox` を付け、`fill`/`stroke` は濃い色（例 `#333`）にする
  （表示枠は白背景なので明暗どちらのテーマでも見える）。JS文字列は
  **シングルクォート**で囲み、SVG内はダブルクォートを使う。
- 複雑な地図・写真・実データのグラフは無理に図にせず、文字で言い換える。
